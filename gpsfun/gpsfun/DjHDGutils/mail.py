...
def send_mail(
        ...
        auth_user=None, auth_password=None, content_subtype='plain', attachments=None):
    """ send mail """
...
def log_mail(subject, message, from_email, unsafe_recipient_list):
    """ log mail """
    fd = open(f"{settings.SITE_ROOT}outbox/msg-{datetime.now()}", "w")

    from_email_ = str(from_email.encode("UTF-8"))
    lst = str(",".join(unsafe_recipient_list).encode("UTF-8"))
    subject_ = str(subject.encode("UTF-8"))

    fd.write(f"From: {from_email_}\r\n")
    fd.write(f"To: {lst}\r\n")
    fd.write(f"Subject: {subject_}\r\n\r\n")
