...
class TableController(object):
    ...
    def process_request(self, skip_paginator=False):
        ...
        if self.request.GET.has_key('page') and not skip_paginator:
        ...
        if self.request.GET.has_key('csv') and self.table.csv_allow:
    ...
    def download_csv(self, request):
        ...
        response['Content-Disposition'] = 'attachment; filename=%s_%s.csv' % (
            self.table.id,
            format(datetime.now(), settings.DATETIME_FORMAT))

        writer = UnicodeWriter(response)
        writer.writerow([cell.html_title() for key, cell in self.iter_title()])
        for row in self.get_paginated_rows():
            writer.writerow([re.sub(r'\n\r|\r\n|\r|\n',
                                    ' ',
                                    unicode(strip_tags(cell.as_html().replace('&nbsp', ' ')))) for cell in row])
        return response
