...
WIKI_URL = WIKI_HOME_URL = "{redmine}/projects/"\
           "{project}/wiki/".format(redmine=REDMINE_URL,
...
class WikiPagesParser(HTMLParser):
    ...
    def handle_data(self, data):
        if self.last_tag == 'a' and self.in_pages_hierarchy:
            if data.strip():
                self.wiki_pages.append((dict(self.last_tag_attrs)['href'],
                                        data.strip()))
...
def login(browser):
    while True:
        ...
        print('Logged in to {0}.'.format(REDMINE_URL))
...
def read_from_redmine(browser):
    ...
    print('Finished reading wiki index, found {0} pages'.format(pages_n))
        ...
        if compared[f] == FileStates.REMOTELY_DELETED:
            ...
            while True:
                ...
                if c == 'l':
                    ...
                    subprocess.call("diff {0} {1}".format(
                        f + FRESH_EXT, f), shell=True)
                elif c == 'n':
                    omit_files.append(f)
                    break
...
def reindex(filelist, omit_files, filename=METAFILE):
    ...
    with open(filename, 'w') as indexinfo:
        for fname in filelist:
            ...
            st = os.stat(fname)
            indexinfo.write("{0:.2f} {1} {2}\n".format(
                st.st_mtime, md5.hexdigest(), fname))

        for entry in oldmeta:
            if entry.fname in omit_files:
                indexinfo.write("{0} {1} {2}\n".format(
                    entry.mtime, entry.md5, entry.fname))
