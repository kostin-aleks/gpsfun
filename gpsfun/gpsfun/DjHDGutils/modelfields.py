...
class MediaPathFieldForm(ChoiceField):
    def __init__(self, path, match=None, recursive=False, required=True,
        ...
        super(MediaPathFieldForm, self).__init__(choices=(), required=required,
...
class MediaFilePathField(models.FilePathField):
    ...
    def contribute_to_class(self, cls, name):
        ...
        setattr(cls, 'get_%s_url' % self.name, get_url)

        def get_abspath(obj):
            from django.conf import settings
            return settings.MEDIA_ROOT+getattr(obj, name)
        setattr(cls, 'get_%s_abspath' % self.name, get_abspath)
